...
def try_generate_pdos_dat_files(location):
    ...
    if all(os.path.isfile(os.path.join(location, f)) for f in required_files):
        ...
        try:
            # First generate TDOS (option 11, then 111)
            print("Step 1: Generating TDOS.dat (11 -> 111)...")
            result1 = subprocess.run('(echo 11; sleep 1; echo 111) | vaspkit', 
                          shell=True, cwd=location, check=False, 
                          stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            
            # Check if TDOS.dat was created
            tdos_path = os.path.join(location, 'TDOS.dat')
            if os.path.isfile(tdos_path):
                print("✓ TDOS.dat generated successfully")
            else:
                print("⚠ TDOS.dat not found after vaspkit execution")
                print(f"vaspkit stdout: {result1.stdout}")
                print(f"vaspkit stderr: {result1.stderr}")
            
            # Now generate PDOS files (option 11, then 113)
            print("Step 2: Generating PDOS files (11 -> 113)...")
            result2 = subprocess.run('(echo 11; sleep 1; echo 113) | vaspkit', 
                          shell=True, cwd=location, check=False,
                          stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            
            # Check if PDOS files were created
            pdos_files = [f for f in os.listdir(location) if f.startswith('PDOS_') and f.endswith('.dat')]
            if pdos_files:
                print(f"✓ Generated {len(pdos_files)} PDOS files")
                return True
            else:
                print("⚠ No PDOS files found after vaspkit execution")
                print(f"vaspkit stdout: {result2.stdout}")
                print(f"vaspkit stderr: {result2.stderr}")
                # Still return True if TDOS was generated
                return os.path.isfile(tdos_path)
