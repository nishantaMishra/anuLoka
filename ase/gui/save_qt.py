...
def save_dialog(gui, filename=None):
    ...
    if not filename:
        ...
        # Fall back to Qt file dialog
        if not filename:
            try:
                from PyQt5.QtWidgets import QFileDialog
                parent_widget = getattr(gui.window, 'win', None)
                dialog_title = _('Save ...')
                start_dir = initialdir or ''
                if initialfile and start_dir:
                    start_dir = os.path.join(start_dir, initialfile)
                elif initialfile:
                    start_dir = initialfile
                filename, _filt = QFileDialog.getSaveFileName(parent_widget, dialog_title, start_dir)
            except Exception:
                pass
