...
"""Qt version of graphs plotting dialog."""
...
class Graphs:
    def __init__(self, gui):
        self.win = ui.Window('Graphs')  # Store reference to prevent GC
        self.expr = ui.Entry('', 50, self.plot)
        self.win.add([self.expr, ui.helpbutton(graph_help_text)])

        self.win.add([ui.Button(_('Plot'), self.plot, 'xy'),
                 ' x, y1, y2, ...'], 'w')
        self.win.add([ui.Button(_('Plot'), self.plot, 'y'),
                 ' y1, y2, ...'], 'w')
        self.win.add([ui.Button(_('Save'), self.save)], 'w')
